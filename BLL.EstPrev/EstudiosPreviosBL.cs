...
namespace BLL
{
    public class EstudiosPreviosBL : absBLL
    {
        ...
        public vProyectos Get(string idproyecto)
        {
            using (ctx = new Entities())
            {
                Mapper.CreateMap<PROYECTOS, vProyectos>();
                PROYECTOS proyecto = ctx.PROYECTOS.Where(t => t.PROYECTO == idproyecto).First();
                vProyectos rProyecto = new vProyectos();
                Mapper.Map(proyecto,rProyecto);
                return rProyecto;
            }
        }
        ...
        public IList<vPOLIZAS> GetPOLIZAS()
        {
            using (ctx = new Entities())
            {
                var lst =  (from  t in ctx.POLIZAS 
                            where t.EST_POL=="AC"
                            select (new vPOLIZAS { COD_POL = t.COD_POL, NOM_POL = t.NOM_POL.ToUpper(), DESCRIPCION=t.DESCRIPCION, VALOR=t.VALOR, PLAZO=t.PLAZO }));
                return lst.ToList();
            }
        }
        ...
        public List<vDEPENDENCIA> GetAllDependencias()
        {
            using (ctx = new Entities())
            {
                Mapper.CreateMap<DEPENDENCIA, vDEPENDENCIA>();
                List<vDEPENDENCIA> or = new List<vDEPENDENCIA>();
                List<DEPENDENCIA> o = ctx.DEPENDENCIA.Where(t => t.ESTADO == "AC" && t.COD_DEP != "00").OrderBy(t => t.NOM_DEP).ToList();
                Mapper.Map(o, or);
                return or;
            }
        }
