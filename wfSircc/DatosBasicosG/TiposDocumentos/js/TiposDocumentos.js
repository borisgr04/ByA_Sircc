var objregistro = (function () {
    ...
    var urlToAbrir = "/Servicios/DatosBasicosG/wsTiposDocumentos.asmx/Get";
    var urlToGuardarNuevo = "/Servicios/DatosBasicosG/wsTiposDocumentos.asmx/Insert";
    var urlToGuardarMod = "/Servicios/DatosBasicosG/wsTiposDocumentos.asmx/Update";
    var urlToEtapas = "/Servicios/DatosBasicosG/wsEtapas.asmx/GetsActivas";
    ...
    var _esValido = function () {
        ...
        var _MensajeFinalValidacion = function () {
            ...
            if (error) {
                ...
                $("#LbMsg").msgBox({ titulo: "Tipos Documentos", mensaje: mensaje, tipo: false });
            } else {
                $("#LbMsg").html("");
            }
        };
    ...
    var _crearElements = function () {
        var sourceEtapas = byaPage.getSource(urlToEtapas);
        $("#cboCOD_ETAPA").byaCombo({ DataSource: source, Value: "COD_ETA", Display: "NOM_ETA" });
    };
    ...
    var _guardarNuevo = function () {
        if (_esValido()) {
            ...
            byaPage.POST_Sync(urlToGuardarNuevo, jsonData, function (result) {
                ...
                $(msgPpal).msgBox({ titulo: "Tipos Documentos", mensaje: byaRpta.Mensaje, tipo: !byaRpta.Error });
                if (!byaRpta.Error) {
                    $("#nuevoButton").byaSetHabilitar(true);
    ...
    var Editar = function () {
        ...
        $(msgPpal).msgBox({ titulo: "Tipos Documentos", mensaje: "Después de modificar los datos y presione el botón Guardar...!!!", tipo: "info" });
        ejecutar = _guardarMod;
    };
    var _Abrir = function () {
        if (idObj == "") {
            $(msgPpal).msgBox({ titulo: "Tipos Documentos", mensaje: "Debe especificar el año", tipo: false });
            return false;
        }
        ...
        $.ajax({
            ...
            async: false,
            beforeSend: function () {
                $(msgPpal).msgBox({ titulo: "Tipos Documentos", mensaje: "Procesando.. espere por favor...!!!", tipo: "info" });
            },
            success: function (result) {
                var e = byaPage.retObj(result.d);
                if (e == null) {
                    $(msgPpal).msgBox({ titulo: "Tipos Documentos", mensaje: "El N° Identificación no se encuentra registrado...!!!", tipo: "warning" });
                    $("#guardarButton").byaSetHabilitar(false);
                    $("#nuevoButton").byaSetHabilitar(true);
                    $("#editarButton").byaSetHabilitar(false);
                    $("#cancelarButton").byaSetHabilitar(false);
                    ...
                    $(msgPpal).msgBox({ titulo: "Tipos Documentos", mensaje: "Se cargaron los datos", tipo: "info" });
                }
            },
...
$(function () {
    byaSite.SetModuloP({ TituloForm: "Tipos Documentos", Modulo: "Gestión", urlToPanelModulo: "gTiposDocumentos.aspx", Cod_Mod: "DTBS4", Rol: "DTBS4TipDocumento" });
    objregistro.init();
});
