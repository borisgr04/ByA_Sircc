var objregistro = (function () {
    ...
    var urlToAbrir = "/Servicios/DatosBasicosG/wsTiposContratos.asmx/Get";
    var urlToGuardarNuevo = "/Servicios/DatosBasicosG/wsTiposContratos.asmx/Insert";
    var urlToGuardarMod = "/Servicios/DatosBasicosG/wsTiposContratos.asmx/Update";
    ...
    var _esValido = function () {
        ...
        var _MensajeFinalValidacion = function () {
            ...
            if (error) {
                ...
                $("#LbMsg").msgBox({ titulo: "Tipos de contratos", mensaje: mensaje, tipo: false });
            } else {
                $("#LbMsg").html("");
            }
        };
    ...
    var _guardarNuevo = function () {
        if (_esValido()) {
            ...
            byaPage.POST_Sync(urlToGuardarNuevo, jsonData, function (result) {
                ...
                $(msgPpal).msgBox({ titulo: "Tipos de contratos", mensaje: byaRpta.Mensaje, tipo: !byaRpta.Error });
                if (!byaRpta.Error) {
                    $("#nuevoButton").byaSetHabilitar(true);
    ...
    var Editar = function () {
        ...
        $(msgPpal).msgBox({ titulo: "Tipos de contratos", mensaje: "Después de modificar los datos y presione el botón Guardar...!!!", tipo: "info" });
        ejecutar = _guardarMod;
    };
    var _Abrir = function () {
        if (idObj == "") {
            $(msgPpal).msgBox({ titulo: "Tipos de contratos", mensaje: "Debe especificar el año", tipo: false });
            return false;
        }
        ...
        $.ajax({
            ...
            async: false,
            beforeSend: function () {
                $(msgPpal).msgBox({ titulo: "Tipos de contratos", mensaje: "Procesando.. espere por favor...!!!", tipo: "info" });
            },
            success: function (result) {
                var e = byaPage.retObj(result.d);
                if (e == null) {
                    $(msgPpal).msgBox({ titulo: "Tipos de contratos", mensaje: "El N° Identificación no se encuentra registrado...!!!", tipo: "warning" });
                    $("#guardarButton").byaSetHabilitar(false);
                    $("#nuevoButton").byaSetHabilitar(true);
                    $("#editarButton").byaSetHabilitar(false);
                    $("#cancelarButton").byaSetHabilitar(false);
                    ...
                    $(msgPpal).msgBox({ titulo: "Tipos de contratos", mensaje: "Se cargaron los datos", tipo: "info" });
                }
            },
...
$(function () {
    byaSite.SetModuloP({ TituloForm: "Tipos de contratos", Modulo: "Gestión", urlToPanelModulo: "gTiposContratos.aspx", Cod_Mod: "DTBS4", Rol: "DBES4TiposCont" });
    objregistro.init();
});
