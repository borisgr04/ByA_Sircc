var objregistro = (function () {
    ...
    var urlToAbrir = "/Servicios/DatosBasicosG/wsPolizas.asmx/Get";
    var urlToGuardarNuevo = "/Servicios/DatosBasicosG/wsPolizas.asmx/Insert";
    var urlToGuardarMod = "/Servicios/DatosBasicosG/wsPolizas.asmx/Update";
    ...
    var _esValido = function () {
        ...
        var _MensajeFinalValidacion = function () {
            ...
            if (error) {
                ...
                $("#LbMsg").msgBox({ titulo: Titulo, mensaje: mensaje, tipo: false });
            } else {
                $("#LbMsg").html("");
            }
        };
    ...
    var _guardarNuevo = function () {
        if (_esValido()) {
            ...
            byaPage.POST_Sync(urlToGuardarNuevo, jsonData, function (result) {
                ...
                $(msgPpal).msgBox({ titulo: Titulo, mensaje: byaRpta.Mensaje, tipo: !byaRpta.Error });
                if (!byaRpta.Error) {
                    $("#nuevoButton").byaSetHabilitar(true);
    ...
    var Editar = function () {
        ...
        $("#guardarButton").byaSetHabilitar(true);
        $("#cancelarButton").byaSetHabilitar(true);
        $("#txtCOD_POL").byaSetHabilitar(false);

        $(msgPpal).msgBox({ titulo: Titulo, mensaje: "Después de modificar los datos y presione el botón Guardar...!!!", tipo: "info" });
        ejecutar = _guardarMod;
    };
    var _Abrir = function () {
        if (idObj == "") {
            $(msgPpal).msgBox({ titulo: Titulo, mensaje: "Debe especificar el año", tipo: false });
            return false;
        }
        ...
        $.ajax({
            ...
            async: false,
            beforeSend: function () {
                $(msgPpal).msgBox({ titulo: Titulo, mensaje: "Procesando.. espere por favor...!!!", tipo: "info" });
            },
            success: function (result) {
                var e = byaPage.retObj(result.d);
                if (e == null) {
                    $(msgPpal).msgBox({ titulo: Titulo, mensaje: "El N° Identificación no se encuentra registrado...!!!", tipo: "warning" });
                    $("#guardarButton").byaSetHabilitar(false);
                    $("#nuevoButton").byaSetHabilitar(true);
                    $("#editarButton").byaSetHabilitar(false);
                    $("#cancelarButton").byaSetHabilitar(false);
                    ...
                    $(msgPpal).msgBox({ titulo: Titulo, mensaje: "Se cargaron los datos", tipo: "info" });
                }
            },
...
$(function () {
    byaSite.SetModuloP({ TituloForm: "Pólizas", Modulo: "Gestión", urlToPanelModulo: "gPolizas.aspx", Cod_Mod: "DTBS4", Rol: "DTBS4Polizas" });
    objregistro.init();
});
